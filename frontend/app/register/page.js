      ...
      <div className="card" style={{ width: '100%', maxWidth: 400, padding: 32 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 24 }}>
          ...
          <div>
            ...
            <div className="mono" style={{ fontSize: 10, color: 'var(--text-dim)' }}>CREATE OPERATOR</div>
          </div>
        </div>

        <h1 style={{ fontSize: 22, fontWeight: 700, marginBottom: 8 }}>Create account</h1>
        <p style={{ fontSize: 13, color: 'var(--text-muted)', marginBottom: 24 }}>
          Register with email and password to access the forensic console.
        </p>
        ...
        <form onSubmit={onSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          <div>
            ...
            <input type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder="Operator name" style={inputStyle} />
          </div>
          <div>
            <label className="mono" style={{ fontSize: 11, color: 'var(--text-dim)', display: 'block', marginBottom: 6 }}>
              EMAIL
